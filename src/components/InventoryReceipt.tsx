...
const CATEGORY_OPTIONS = ['General', 'Grocery', 'Vegetables', 'Sweets', 'Snacks', 'Hardware', 'Electrical', 'Dairy', 'Masale', 'Dry Fruits', 'Non-Veg', 'Medical', 'Stationery', 'Cosmetics'];
  ...
  return (
    <div className="split-layout">
      ...
      <div className="split-col">
        <div className="glass-panel">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl sm:text-2xl font-semibold gradient-amber">🏪 Dukandar Inventory</h2>
          ...
          <div className="section-title"><span>🧾 Billing & Export</span><button className="retro-btn text-[10px]" onClick={() => setShortcutsHelp(true)}>⌨️ Shortcuts</button></div>
          <div className="flex flex-col gap-3">
            <div className="input-row">
              <div className="input-group">
          ...
          <div className="section-title">
            ...
            <div className="flex gap-1.5 flex-wrap">
              <button className={`retro-btn ${viewMode === 'card' ? 'active' : ''}`} style={viewMode === 'card' ? { background: '#ffb347', color: '#000', borderColor: '#ffb347' } : {}} onClick={() => setViewMode('card')}>📱 Cards</button>
              <button className={`retro-btn ${viewMode === 'table' ? 'active' : ''}`} style={viewMode === 'table' ? { background: '#ffb347', color: '#000', borderColor: '#ffb347' } : {}} onClick={() => setViewMode('table')}>📊 Table</button>
              <button className="retro-btn" onClick={() => setShowBulk(!showBulk)}>📋 Bulk</button>
              <button className="retro-btn" onClick={() => setShowCost(!showCost)}>{showCost ? '💰 Hide Cost' : '💰 Show Cost'}</button>
          ...
          <button className="print-btn mt-4" onClick={handlePrint}>📠 Print Invoice</button>
          ...
          <div className="btn-group justify-center mt-3">
            ...
            <button className="retro-btn" onClick={shareWhatsApp}>📱 WhatsApp</button>
            <button className="retro-btn" onClick={saveReceipt}>💾 Save</button>
          </div>

          <div className="btn-group justify-center mt-2">
            <button className="retro-btn" onClick={() => setShowHistory(!showHistory)}>📜 History ({receiptHistory.length})</button>
            <button className="retro-btn" onClick={exportAllJSON}>📥 Export</button>
          ...
          {shortcutsHelp && (
            <div className="mt-3 bg-[#0f0f18] border border-[#2c2c38] rounded-xl p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-amber-400">⌨️ Keyboard Shortcuts</span>
      ...
      <div className="split-col center">
        ...
        <p className="text-[11px] text-gray-500 text-center px-2">⬆️ Line-by-line + CGST/SGST + QR + WhatsApp + History</p>
      </div>
    </div>
  );
}
